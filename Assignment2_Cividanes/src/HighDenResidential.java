import java.util.Random;

import org.newdawn.slick.Color;
import org.newdawn.slick.Graphics;
/*
 * Author: Morgan Cividanes
 * Date: 02/23/2021
 * Description: The HighDenResidential Class -
 *40% - HighDenResidential
 *20% - AverageDenResidential
 *0% - LowDenResidential
 *20% - Commercial
 *10% - Industrial
 *10% - RecreationSpace
 * Should be dark gray in color
 */
public class HighDenResidential extends Zone
{
	private int width;
	private int height;
	private Color color;
	public HighDenResidential() //Constructor
	{
		this.width = width;
		this.height = height;
		this.color = color.darkGray;
	}

	@Override
	public void drawSelf(int xPos, int yPos, Graphics painter, int pixWidth, int pixHeight) 
	{
		painter.setColor(color.darkGray);//Setting the color
		painter.drawRect(xPos * pixWidth, yPos * pixHeight, pixWidth, pixHeight);//Drawing the rectangle
		painter.fillRect(xPos, yPos, pixWidth, pixHeight);//Fill the Rectangle with color
	}

	@Override
	public int generateNeighbor() //GENERATES NEXT RANDOM NEIGHBOR BASED ON PROBABILITIES LISTED IN DESCRIPTION
	{
		//Creating new random integer from 1-100
		Random random = new Random();
		int randomConv = random.nextInt(100) + 1; 
		int neighbor = 0;
		if(randomConv < 41)
		{
			neighbor = 4; //high den
		}
		else if(randomConv > 40 && randomConv < 61)
		{
			neighbor = 2; //average den
		}
//		else if(randomConv > 60 && randomConv < 56)
//		{
//			neighbor = 3; //low den
//		}
		else if(randomConv > 60 && randomConv < 81)
		{
			neighbor = 1; //commerical
		}
		else if(randomConv > 80 && randomConv < 91)
		{
			neighbor = 5; //industrial
		}
		else if(randomConv > 90)
		{
			neighbor = 6; //rec space
		}
		return neighbor;
	}

}
